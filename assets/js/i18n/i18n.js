...
(function () {
  ...
  const spanishDomMap = [
    ...
    ["At OVI Wellness, we operate under the strictest standards of medical safety and absolute compliance with Florida state licensing regulations. All treatments are prescribed and monitored by fully licensed medical professionals following rigorous, evidence-based diagnostic evaluation.", "En OVI Wellness operamos bajo los estándares más estrictos de seguridad médica y cumplimiento de licencias en el estado de Florida. Todos los tratamientos son prescriptos y monitoreados por profesionales de la salud licenciados tras una evaluación diagnóstica rigurosa."],
  ...
  async function loadLocaleData() {
    try {
      ...
      if (legalPageRes && legalPageRes.ok) {
        const legalPageData = await legalPageRes.json();
        window.clinicData.pages = window.clinicData.pages || {};
