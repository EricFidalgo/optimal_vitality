...
(function () {
  ...
  function renderTeamPage() {
    ...
    const headlineEl  = document.querySelector('[data-i18n="team.heroHeadline"]');
    const highlightEl = document.querySelector('[data-i18n="team.heroHeadlineHighlight"]');
    ...
    if (highlightEl && t?.heroHeadlineHighlight)  highlightEl.textContent = t.heroHeadlineHighlight;
    ...
    const labels = {
      ...
      professionalAssociations: t?.ui?.professionalAssociations || 'Professional Associations',
    ...
    teamContainer.innerHTML = teamData.map((member, index) => {
    ...
    setTimeout(() => {
      ...
      if (lowerContent) { lowerContent.style.transition = 'opacity 0.25s ease-in-out'; lowerContent.style.opacity = '1'; }
      if (footerRoot)   { footerRoot.style.transition   = 'opacity 0.25s ease-in-out'; footerRoot.style.opacity   = '1'; }
    }, 60);
  }
